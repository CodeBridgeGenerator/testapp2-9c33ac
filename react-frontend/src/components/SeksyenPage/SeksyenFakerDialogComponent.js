...
import seksyenFakerFactory from "./seksyenFakerFactory";
...
const SeksyenFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = seksyenFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("seksyen").create(elem));
...
export default connect(mapState, mapDispatch)(SeksyenFakerDialogComponent);
