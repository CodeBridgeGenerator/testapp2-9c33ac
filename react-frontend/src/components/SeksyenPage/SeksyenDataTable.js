...
const SeksyenDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.nama}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "seksyen"});
...
<Column field="nama" header="nama" body={pTemplate0} filter={selectedFilterFields.includes("nama")} hidden={selectedHideFields?.includes("nama")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload Seksyen Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="seksyen"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Seksyen" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default SeksyenDataTable;