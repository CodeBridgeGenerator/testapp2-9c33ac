...
import memoFakerFactory from "./memoFakerFactory";
...
const MemoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = memoFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("memo").create(elem));
...
export default connect(mapState, mapDispatch)(MemoFakerDialogComponent);
