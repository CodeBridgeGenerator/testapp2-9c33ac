...
import MemoDatatable from "./MemoDataTable";
import MemoEditDialogComponent from "./MemoEditDialogComponent";
import MemoCreateDialogComponent from "./MemoCreateDialogComponent";
import MemoFakerDialogComponent from "./MemoFakerDialogComponent";
import MemoSeederDialogComponent from "./MemoSeederDialogComponent";
...
const MemoPage = (props) => {
    ...
    const filename = "memo.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("memo");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("memo").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Memo", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("memo").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Memo </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="memo-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="memo-datatable">
                    <MemoDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <MemoEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MemoCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <MemoFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MemoSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MemoPage);
